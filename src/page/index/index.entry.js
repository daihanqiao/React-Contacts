 ...
 * @Last Modified time: 2015-12-27 21:33:06
 */
function ready() {
                ...
                success: function(ret) {
                    ...
                    if (needUpVersion && method.toLowerCase() !== 'get') {
                        ...
                        ajax('version', function(ret) {
                            if (ret) {
                                if (ret.length > 0 && versionId) {
                                    ajax('version/' + versionId, function(ret) {}, {
                                        update_time: _.now(),
                                        "_method": "PUT"
                                    }, 'post');
                                } else {
                                    ajax('version', function(ret) {
                                        ret && (versionId = ret.id);
                                    }, {
                                        "update_time": _.now(),
                                    }, 'post');
                                }
    ...
    request('department_class', function(ret) {
        ...
        request('member', function(ret) {
            //只维护该数据，所有操作通过订阅者模式操作该数据，通过render()刷新，props向下分发数据
                ...
                ModalAlert.show({
                    ...
                    okCallBack: function() {
                        ...
                        request('department_class/' + data.id, function(ret) {
                            if (ret) {
                                //删除分组下联系人
                                var member_list = _.result(_.find(memberDepartmentList, {
                                    id: data.id
                                }), 'member_list');
                                if (member_list && member_list.length > 0) {
                                    ...
                                    _.map(member_list, function(memberData) {
                                        requestList.push({
                                            "method": "DELETE",
                                            "path": "/mcm/api/member/" + memberData.id
                                        });
                                    });
            ...
            Pubsub.subscribe(EVENT_CFG.editDepartmentEvent, function(evt, data) {
                ...
                ModalAlert.show({
                    ...
                    confirm: 1,
                    namePlaceholder: '分组名',
                    okCallBack: function(nameValue) {
                        if (nameValue) {
                            request('department_class/' + data.id, function(ret) {
                                if (ret) {
                                    _.assign(_.find(memberDepartmentList, {
                                        id: data.id
                                    }), ret);
                                    render();
                                }
                            }, {
                                department_name: nameValue,
                                "_method": "PUT"
                            }, 'post');
                        } else {
            ...
            Pubsub.subscribe(EVENT_CFG.addMemberEvent, function(evt, data) {
                ...
                ModalAlert.show({
                    ...
                    mobilePlaceholder: '联系电话',
                    okCallBack: function(nameValue, mobileValue) {
            ...
            function render() {
                ReactDOM.render(
                    <DepartmentList departmentDataList={memberDepartmentList}></DepartmentList>,
                    document.getElementById('departmentCon')
                );
            }
            ReactDOM.render(
                <AddGroup ></AddGroup>,
                document.getElementById('addGroupCon')
            );
        });
    })
}
